...
fn sources_re() -> &'static Regex {
    ...
    RE.get_or_init(|| Regex::new(r#"sources:\s*\[\{file:\s*["'](?P<url>[^"']+)"#).unwrap())
}

pub struct LuluStreamExtractor {
    pub base: BaseExtractor,
    chrome_client: rquest::Client,
}

impl LuluStreamExtractor {
...
#[async_trait]
impl Extractor for LuluStreamExtractor {
    ...
    async fn extract(
        ...
        let final_url = sources_re()
            .captures(&html)
            .and_then(|c| c.name("url"))
            .map(|m| m.as_str().to_string())
            .ok_or_else(|| ExtractorError::extract("LuluStream: source URL not found"))?;
