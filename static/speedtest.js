...
class MediaFlowSpeedTest {
    ...
    initializeForm() {
        ...
        // The top "MediaFlow API Password" field is the source of truth.
        // Hydrate it from localStorage (shared across pages — set in the
        // URL generator's globalApiPassword) and persist edits back.
        // The per-server `.server-password` field is treated as an optional
        // override (see collectServerConfigurations), so a single-server
        // user only has to fill the top field.
    ...
    collectServerConfigurations() {
        ...
        this.servers = [];
        const serverInputs = document.querySelectorAll('.server-input');
        // Fall back to the top "MediaFlow API Password" field when a server
        // row's per-server override is blank — saves single-server users
        // from having to type the password twice.
